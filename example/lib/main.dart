...
import 'package:tts_example/data.dart';
...
class _MyAppState extends State<MyApp> {
  ...
  static final scriptTagRegex = RegExp(r'\[.*?\]');
  static final tagRegex = RegExp(r'\<.*?\>');
  static final nonAlphaWithPossessiveRegEx = RegExp(
  ...
  String _normalizeWord(String word) {
    final normalized = word
        ...
        .replaceAll(tagRegex, '')
        .replaceAll(nonAlphaWithPossessiveRegEx, '')
        .toLowerCase();
    return trim(normalized, "'");
  }

  static String trim(String input, String char) {
    final matcher = CharMatcher.charSet(char);
    return matcher.trimFrom(input);
  }
      ...
      switch (_language) {
        case Langage.id:
          ...
          for (var wordText in wordTexts) {
            var normalise = _normalizeWord(wordText);
        ...
        case Langage.sw:
          ...
          for (final word in words) {
            var normalise = _normalizeWord(word);
            final ipa = await _findIPA(normalise, 'sw');
            wordIPAs.add(ipa);
        ...
        default:
          if (!_testPerformance) {
            ...
            for (final word in words) {
              var normalise = _normalizeWord(word);
              final ipa = await _findIPA(normalise, 'en');
              wordIPAs.add(ipa);
            ...
            for (final inputIds in testInputIds) {
              ...
              final executeTime = stopwatch.elapsedMilliseconds;
              measureTimes.add(executeTime);
              stopwatch.stop();
              print('output: $output');
              setState(() {
                _result += 'Duration:\n ${output.join('\n')}';
            ...
            final avgTime =
                measureTimes.reduce((a, b) => a + b) / measureTimes.length;
            setState(() {
              _result += '''\n
min = $minTime
max = $maxTime
...
''';
  ...
  Future<void> init() async {
    ...
    wordDb = List<Map>.from(jsonDecode(assetContent));
  }
      ...
      home: Scaffold(
        ...
        body: Center(
          ...
          children: [
            ...
            ListTile(
              ...
              leading: Radio<Langage>(
                ...
                onChanged: (Langage? value) {
                  setState(() {
                    _language = value;
                    _textController.text = defaultEnText;
                  });
                },
              ),
            ),
            ListTile(
              ...
              leading: Radio<Langage>(
                ...
                onChanged: (Langage? value) {
                  setState(() {
                    _language = value;
                    _textController.text = defaultIdText;
                    _testPerformance = false;
                  });
                },
              ),
            ),
            ListTile(
              ...
              leading: Radio<Langage>(
                ...
                onChanged: (Langage? value) {
                  setState(() {
                    _language = value;
                    _textController.text = defaultSwText;
                    _testPerformance = false;
                  });
                },
              ),
            ...
            CheckboxListTile(
              value: _testPerformance,
              onChanged: (val) {
                setState(() {
                  _testPerformance = val == true;
                });
              },
